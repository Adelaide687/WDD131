...
function filterFunction(recipe, searchInput) {
...
function filter() {
	...
	const filtered = recipes.filter(recipe => filterFunction(recipe, searchInput));
...
function searchHandler(e) {
	...
	const html = filtered.map(recipeTemplate).join('');
...
window.addEventListener('DOMContentLoaded', () => {
	...
	document.getElementById('recipe-container').innerHTML = recipeTemplate(randomRecipe);
});
